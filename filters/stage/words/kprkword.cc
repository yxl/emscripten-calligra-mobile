...
KoFilter::ConversionStatus KprWords::convert(const QByteArray& from, const QByteArray& to)
{
    if (to != "application/x-words" || from != "application/x-kpresenter")
    ...
    kwdoc.setAttribute("editor", "KprWords converter");
...
void KprWords::convert()
{
    ...
    foreach(KprObject* it, objList) {
        ...
        //kDebug() <<"KprWords::convert y=" << it->y <<" ptPageHeight=" << ptPageHeight
        ...
        for (uint item = 0; item < lstcount ; ++item) {
            ...
            for (; !textElem.isNull() ; textElem = textElem.nextSibling().toElement()) {
                ...
                text += textElem.text();
                //kDebug() <<"KprWords::convert text now" << text;
